...
我们只考虑良好的执行。一个执行`E = < P, A, po, so, W, V, sw, hb >`是格式良好的，如果下面的条件为真：

1. 在执行中每个读看到对同一个变量的写。

    所有`volatile`变量的读和写都是`volatile`操作。对于A中的所有读取，我们有`W(r)`在A中并且`W(r).v = r.v`。变量`r.v`是volatile的，当且仅当`r`是一个volatile读取，并且变量`w.v`是volatile的，当且仅当`w`是一个volatile写。

2. `happens-before`顺序是一个偏序。

    `happens-before`顺序由`synchronizes-with`边缘和程序顺序的传递闭包给出。它必须是一个有效的偏序：自反，传递和反对称。

3. 执行遵循线程内的一致性。

    对于每个线程`t`，由`t`在A执行的操作是相同的，就如由那个线程在程序顺序中隔离地生成的，和每个写`w`写入值`V(w)`，假定每个读`r`看到值`V(W(r))`。每个读看到的值由内存模型决定。给出的程序顺序必须根据`P`的线程内语义反映操作将被执行的程序的顺序。

4. 执行是`happens-before`一致的 ([§17.4.6](http://docs.oracle.com/javase/specs/jls/se7/html/jls-17.html#jls-17.4.6))。

5. 执行遵循同步顺序的一致性。

    对于A中所有的volatile读，它不是这种情况：不是`so(r, W(r))`，就是A中存在一个写`w`以致`w.v = r.v` 并且 `so(W(r), w)` 并且 `so(w, r)`。
    
17.4.8. Executions and Causality Requirements 和 17.4.9. Observable Behavior and Nonterminating Executions 请参照原文。
